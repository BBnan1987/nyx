...
class MemoryModel {
  ...
  _generateCursorClass() {
    ...
    const getterLines = fieldInfos.map(f => {
      ...
      return `    get ${f.name}() { return this._v.${method}(this._off + ${f.offset}, this._isLittleEndian); }`;
    });
    ...
    const setterLines = fieldInfos.map(f => {
      ...
      return `    set ${f.name}(v) { this._v.${method}(this._off + ${f.offset}, v, this._isLittleEndian); }`;
    });
  ...
  _generateObjectClass() {
    ...
    const getterLines = fieldInfos.map(f => {
      ...
      // Regular field
      const method = typeInfo[f.type].read;
      return `    get ${f.name}() { return this._v.${method}(${f.offset}, this._isLittleEndian); }`;
    });
    ...
    const setterLines = fieldInfos.map(f => {
      ...
      // Regular field
      const method = typeInfo[f.type].write;
      return `    set ${f.name}(v) { this._v.${method}(${f.offset}, v, this._isLittleEndian); }`;
    });
