...
class MemoryModel {
  constructor(name, fields, parent, options = {}) {
    ...
    // Collect model definitions recursively
    const modelMap = {};
  ...
  _generateCursorClass() {
    ...
    const getterLines = fieldInfos.map(f => {
      ...
      return new this._models.${f.model.name}.CursorClass(this.$address + ${f.offset}n, slice, 1, this.size, this._models);
    }`;
      }
  ...
  read(address) {
    ...
    const obj = new this.CursorClass(BigInt(address), buffer, 1, this.size, this._modelMap);
  ...
  readArray(address, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const view = new DataView(slice.buffer, slice.byteOffset, slice.byteLength);
      const cursor = new this.CursorClass(BigInt(address) + BigInt(i * this.size), slice, count, this.size, this._modelMap);
