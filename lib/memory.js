...
class MemoryModel {
  constructor(name, fields, parent) {
    this.name = name;
    ...
    for (const f of fields) {
      ...
      if (f.model && f.type !== 'pointer') {
        ...
        const align = f.model.size > 8 ? 8 : f.model.size;
      ...
      if (f.type === 'pointer' && f.model) {
        if (!f.name) throw new Error('Model field requires name property');
        offset = Math.ceil(offset / 8) * 8; // Align to 8
        fieldInfos.push({
          name: f.name,
          type: 'model_pointer',
          model: f.model,
      ...
      const align = info.size > 8 ? 8 : info.size;
      offset = Math.ceil(offset / align) * align;
      fieldInfos.push({ name: f.name, type: f.type, offset, size: info.size });
      offset += info.size;
    }
    this.size = Math.ceil(offset / 8) * 8; // Align struct to 8 bytes
    this.fieldInfos = fieldInfos;

    // Generate cursor classes
    this._generateCursorClass();
    this._generateFastCursorClass();
    this._generateObjectClass();
  }

  // Generate DataView-based cursor class
  _generateCursorClass() {
    const { fieldInfos } = this;

    const getterLines = fieldInfos.map(f => {
      // Inline model: return nested object
      if (f.type === 'model_inline') {
        return `    get ${f.name}() {
      const slice = this._b.subarray(this._off + ${f.offset}, this._off + ${f.offset} + ${f.size});
      const view = new DataView(slice.buffer, slice.byteOffset, slice.byteLength);
      return new this._models.${f.model.name}.ObjectClass(this.$address + ${f.offset}n, slice, view, this._models);
    }`;
      }
      // Pointer to model - return from cache (eagerly loaded)
      ...
      return `    get ${f.name}() { return this._v.${method}(this._off + ${f.offset}, true); }`;
    });

    const setterLines = fieldInfos.map(f => {
      // Models don't have setters (read-only for now)
      if (f.type === 'model_inline' || f.type === 'model_pointer') {
        return `    set ${f.name}(v) { throw new Error('Cannot set model field'); }`;
      }
      // Regular field
      const method = typeInfo[f.type].write;
      return `    set ${f.name}(v) { this._v.${method}(this._off + ${f.offset}, v, true); }`;
    });

    // Build model map for nested models (recursive)
    const modelMap = {};
    const collectModels = (fields) => {
      for (const f of fields) {
        if ((f.type === 'model_inline' || f.type === 'model_pointer') && !modelMap[f.model.name]) {
          modelMap[f.model.name] = f.model;
          collectModels(f.model.fieldInfos);
        }
      }
    };
    collectModels(fieldInfos);
    ...
    const cursorCode = `
      return class Cursor_${this.name} {
        constructor(baseAddress, buffer, count, modelSize, models) {
          ...
          this._b = buffer;
          this._v = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
          this._count = count;
    ...
    this.CursorClass = new Function('binding', 'DataView', cursorCode)(binding, DataView);
  ...
  _generateFastCursorClass() {
    ...
    for (const f of fieldInfos) {
      // Model fields always need DataView
      if (f.type === 'model_inline' || f.type === 'model_pointer') {
        needsDataView = true;
        continue;
      }
    ...
    const getterLines = fieldInfos.map(f => {
      ...
      if (shift !== null) {
        ...
        const method = typeInfo[f.type].read;
        return `    get ${f.name}() { return this._dv.${method}(this._off + ${f.offset}, true); }`;
      }
    });

    // Generate setters
    const setterLines = fieldInfos.map(f => {
      // Models don't have setters
      if (f.type === 'model_inline' || f.type === 'model_pointer') {
        return `    set ${f.name}(v) { throw new Error('Cannot set model field'); }`;
      }
      ...
      if (shift !== null) {
        ...
        const method = typeInfo[f.type].write;
        return `    set ${f.name}(v) { this._dv.${method}(this._off + ${f.offset}, v, true); }`;
      }
    });

    const fastCursorCode = `
      return class CursorFast_${this.name} {
        constructor(baseAddress, buffer, count, modelSize, models) {
          this._baseAddr = baseAddress;
          this._b = buffer;
          ${viewSetup.join('\n          ')}
    ...
    this.CursorClassFast = new Function(
      'binding', 'DataView',
    ...
    // Add $toObject to fast cursor prototype, snapshot current element into target or new object
    const model = this;
    this.CursorClassFast.prototype.$toObject = function (target) {
      target = model.snapshot(this, target);
      target._address = this.$address;
      return target;
    };
  ...
  // Generate simple object class for single reads
  _generateObjectClass() {
    const { fieldInfos } = this;
    ...
    const getterLines = fieldInfos.map(f => {
      ...
      if (f.type === 'model_inline') {
        return `    get ${f.name}() {
      const slice = this._b.subarray(${f.offset}, ${f.offset} + ${f.size});
      const view = new DataView(slice.buffer, slice.byteOffset, slice.byteLength);
      return new this._models.${f.model.name}.ObjectClass(this._address + ${f.offset}n, slice, view, this._models);
    }`;
      }
      // Pointer to model, return from cache (eagerly loaded)
      if (f.type === 'model_pointer') {
      ...
      // Regular field
      const method = typeInfo[f.type].read;
      return `    get ${f.name}() { return this._v.${method}(${f.offset}, true); }`;
    });
    ...
    const setterLines = fieldInfos.map(f => {
      ...
      // Regular field
      const method = typeInfo[f.type].write;
      return `    set ${f.name}(v) { this._v.${method}(${f.offset}, v, true); }`;
    });

    const objCode = `
      return class MemObj_${this.name} {
        constructor(address, buffer, view, models) {
        ...
        ${getterLines.join('\n')}
        ${setterLines.join('\n')}
        flush() { binding.writeMemory(this._address, this._b); }
      }
    `;

    this.ObjectClass = new Function('binding', objCode)(binding);

    // Add $toObject to object class prototype, snapshot into target or new object
    const model = this;
    this.ObjectClass.prototype.$toObject = function (target) {
      target = model.snapshot(this, target);
      target._address = this._address;
      return target;
    };
  }
  ...
  read(address) {
    const buffer = binding.readMemoryInto
      ? (() => { const b = new Uint8Array(this.size); binding.readMemoryInto(BigInt(address), b); return b; })()
      : binding.readMemoryFast(BigInt(address), this.size >>> 0);
    const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
    const obj = new this.ObjectClass(BigInt(address), buffer, view, this._modelMap);
    ...
    if (pointerModelFields.length > 0) {
      ...
      for (const field of pointerModelFields) {
        const ptr = view.getBigUint64(field.offset, true);
        obj._pointerModelCache[field.name] = ptr === 0n ? null : field.model.read(ptr);
      }
    }
  ...
  // Create a one-time cursor for count objects (allocates buffer)
  cursor(address, count) {
    const totalSize = this.size * count;
    const buffer = binding.readMemoryFast(BigInt(address), totalSize >>> 0);
    const cursor = new this.CursorClass(BigInt(address), buffer, count, this.size, this._modelMap);
    ...
    if (pointerModelFields.length > 0) {
      ...
      for (const field of pointerModelFields) {
        const ptr = view.getBigUint64(field.offset, true);
        cursor._pointerModelCache[field.name] = ptr === 0n ? null : field.model.read(ptr);
      }
    }
  ...
  // Create a reusable cursor (DataView-based), call $load() to read new data
  createCursor(maxCount) {
    const bufferSize = this.size * maxCount;
    const buffer = new Uint8Array(bufferSize);
    const cursor = new this.CursorClass(0n, buffer, 0, this.size, this._modelMap);
  ...
  // Create a reusable cursor (TypedArray-based), FASTEST option
  createFastCursor(maxCount) {
    const bufferSize = this.size * maxCount;
    const buffer = new Uint8Array(bufferSize);
    const cursor = new this.CursorClassFast(0n, buffer, 0, this.size, this._modelMap);
  ...
  readArray(address, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const view = new DataView(slice.buffer, slice.byteOffset, slice.byteLength);
      const obj = new this.ObjectClass(BigInt(address) + BigInt(i * this.size), slice, view, this._modelMap);
  ...
  initSnapshot(target) {
    for (const field of this.fieldInfos) {
      if (field.type === 'model_inline') {
        const Cls = snapshotRegistry._models[field.model.name];
        target[field.name] = Cls ? new Cls() : {};
        field.model.initSnapshot(target[field.name]);
    ...
    for (const field of this.fieldInfos) {
      if (field.type === 'model_inline') {
        ...
        if (val) {
          ...
          field.model.snapshot(val, target[field.name]);
        } else {
          target[field.name] = null;
        }
      } else if (field.type === 'model_pointer') {
  ...
  static define(name, fields, parent) {
    return new MemoryModel(name, fields, parent);
  }
}
