...
//---- Enable thread_local ImGui context for multi-threaded usage.
// Each thread gets its own GImGui pointer, so multiple threads can use separate ImGui contexts safely.
// See imgui.cpp "Current context pointer" section for details.
struct ImGuiContext;
extern thread_local ImGuiContext* NyxImGuiTLS;
#define GImGui NyxImGuiTLS
...
#define IMGUI_DISABLE_OBSOLETE_KEYIO  // 1.87+ disable legacy io.KeyMap[]+io.KeysDown[] in favor io.AddKeyEvent(). This
                                      // is automatically done by IMGUI_DISABLE_OBSOLETE_FUNCTIONS.
...
#ifdef NYX_IMGUI_V8_INTEGRATION

#include <v8.h>

// forward declerations
inline float GetFloatProperty(v8::Isolate* isolate,
...
#define IM_VEC2_CLASS_EXTRA                                                                                            \
  ImVec2(v8::Isolate* isolate, v8::Local<v8::Value> obj) {                                                             \
    if (obj->IsObject()) {                                                                                             \
      *this = FromObject(isolate, obj.As<v8::Object>());                                                               \
    } else {                                                                                                           \
      x = y = 0.0f;                                                                                                    \
    }                                                                                                                  \
  }                                                                                                                    \
...
#define IM_VEC4_CLASS_EXTRA                                                                                            \
  ImVec4(v8::Isolate* isolate, v8::Local<v8::Value> obj) {                                                             \
    if (obj->IsObject()) {                                                                                             \
      *this = FromObject(isolate, obj.As<v8::Object>());                                                               \
    } else {                                                                                                           \
      x = y = z = 0.0f;                                                                                                \
  ...
  v8::Local<v8::Object> ToObject(v8::Local<v8::Context> context) const {                                               \
    ...
    obj->Set(context, v8::String::NewFromUtf8(isolate, "x").ToLocalChecked(), v8::Number::New(isolate, x)).Check();    \
    obj->Set(context, v8::String::NewFromUtf8(isolate, "y").ToLocalChecked(), v8::Number::New(isolate, y)).Check();    \
    obj->Set(context, v8::String::NewFromUtf8(isolate, "z").ToLocalChecked(), v8::Number::New(isolate, z)).Check();    \
    obj->Set(context, v8::String::NewFromUtf8(isolate, "w").ToLocalChecked(), v8::Number::New(isolate, w)).Check();    \
    return obj;                                                                                                        \
  }

#endif  // NYX_IMGUI_V8_INTEGRATION
