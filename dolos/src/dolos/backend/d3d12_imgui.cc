...
#include "dolos/backend/d3d12_imgui_pixel.h"
#include "dolos/backend/d3d12_imgui_vertex.h"
...
D3D12ImGui::D3D12ImGui() noexcept
    : pd3dDevice(nullptr),
      pRootSignature(nullptr),
      pPipelineState(nullptr),
      RTVFormat(DXGI_FORMAT_UNKNOWN),
      pFontTextureResource(nullptr),
...
bool D3D12ImGui::Initialize(ID3D12Device* device,
  ...
  pFrameResources = new RenderBuffers[num_frames_in_flight];
  ...
  for (int i = 0; i < num_frames_in_flight; i++) {
    RenderBuffers* fr = &pFrameResources[i];
    fr->IndexBuffer = NULL;
...
void D3D12ImGui::Shutdown() {
  fprintf(stdout, "Shutdown ImGui dx12\n");
  // IM_ASSERT(bd != NULL && "No renderer backend to shutdown, or already shutdown?");
  ImGuiIO& io = ImGui::GetIO();

  InvalidateDeviceObjects();
  delete[] pFrameResources;
  io.BackendRendererName = NULL;
}

void D3D12ImGui::NewFrame() {
  // IM_ASSERT(bd != NULL && "Did you call Initialize()?");

  if (!pPipelineState) CreateDeviceObjects();
}

void D3D12ImGui::RenderDrawData(ImDrawData* draw_data,
                                ID3D12GraphicsCommandList* graphics_command_list) {  // Avoid rendering when minimized
  ...
  RenderBuffers* fr = &pFrameResources[frameIndex % numFramesInFlight];
  ...
  if (fr->VertexBuffer == NULL || fr->VertexBufferSize < draw_data->TotalVtxCount) {
    ...
    if (pd3dDevice->CreateCommittedResource(&props,
  ...
  SetupRenderState(draw_data, graphics_command_list, fr);
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != NULL) {
        ...
        graphics_command_list->RSSetScissorRects(1, &r);
...
bool D3D12ImGui::CreateDeviceObjects() {
  if (!pd3dDevice) return false;
  if (pPipelineState) InvalidateDeviceObjects();
  ...
  // Create the root signature
  {
    ...
    if (d3d12_dll == NULL) {
      ...
      const char* localD3d12Paths[] = {
          ".\\d3d12.dll", ".\\d3d12on7\\d3d12.dll", ".\\12on7\\d3d12.dll"};  // A. current directory, B. used by some
    ...
    pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&pRootSignature));
    blob->Release();
  }
  ...
  {
    psoDesc.VS = {g_vertex_shader_bytes, g_vertex_shader_size};
  ...
  psoDesc.PS = {g_pixel_shader_bytes, g_pixel_shader_size};
  ...
  HRESULT result_pipeline_state = pd3dDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pPipelineState));
...
void D3D12ImGui::InvalidateDeviceObjects() {
  if (!pd3dDevice) return;
  ImGuiIO& io = ImGui::GetIO();

  SafeRelease(pRootSignature);
  SafeRelease(pPipelineState);
  SafeRelease(pFontTextureResource);
  io.Fonts->SetTexID(NULL);  // We copied pFontTextureView to
...
void D3D12ImGui::SetupRenderState(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, RenderBuffers* fr) {
  ...
  ctx->SetPipelineState(pPipelineState);
...
void D3D12ImGui::CreateFontsTexture() {
  // Build texture atlas
  ImGuiIO& io = ImGui::GetIO();
  ...
  // Upload texture to graphics system
  {
    ...
    hr = pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence));
    IM_ASSERT(SUCCEEDED(hr));
    ...
    hr = pd3dDevice->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&cmdQueue));
    IM_ASSERT(SUCCEEDED(hr));

    ID3D12CommandAllocator* cmdAlloc = NULL;
    hr = pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&cmdAlloc));
    IM_ASSERT(SUCCEEDED(hr));

    ID3D12GraphicsCommandList* cmdList = NULL;
    hr = pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAlloc, NULL, IID_PPV_ARGS(&cmdList));
    IM_ASSERT(SUCCEEDED(hr));
    ...
    pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, hFontSrvCpuDescHandle);
    SafeRelease(pFontTextureResource);
  ...
  static_assert(sizeof(ImTextureID) >= sizeof(hFontSrvGpuDescHandle.ptr),
                "Can't pack descriptor handle into TexID, 32-bit not supported yet.");
  io.Fonts->SetTexID((ImTextureID)hFontSrvGpuDescHandle.ptr);
}
